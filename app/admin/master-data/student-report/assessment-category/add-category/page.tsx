...
/* ---------- Reusable row (dipisah & memoized supaya stabil) ---------- */
type RowProps = {
  label: string;
  required?: boolean;
  input: React.ReactNode;
};
const FormRow = React.memo<RowProps>(function FormRow({ label, required, input }) {
  return (
...
const AddCategoryPage: React.FC = () => {
  ...
  // Gabungkan status modal untuk overlay & dimming
  const modalOpen = addModalOpen || deleteModalOpen;

  // Stabilkan handler agar input tidak kehilangan fokus
  ...
  const actionDisabled = useMemo(() => {
    return !form.categoryName || !form.assessmentType || !form.relatedTo;
  }, [form]);

  return (
    <div className="relative">
      {/* Overlay penggelap header/sidebar/halaman ketika modal terbuka */}
      {modalOpen && <div className="fixed inset-0 z-40 bg-black/40"></div>}

      {/* Wrapper konten; saat modal terbuka dibuat agak gelap & non-interaktif */}
      <div className={modalOpen ? "pointer-events-none opacity-60" : ""}>
        ...
        <div className="-mx-6 -mt-1 border-b border-gray-100 bg-[#f3f4f6] px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-semibold text-gray-900">Add Assessment Category</h2>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)]">
                ...
                <div className="mt-6 flex flex-wrap gap-3 justify-end">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full bg-[#6c2bd9] px-6 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-[#5922b8] disabled:cursor-not-allowed disabled:opacity-60"
                  >
